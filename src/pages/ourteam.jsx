    ...
    <Container maxWidth="lg" sx={{ mt: { xs: 4, md: 6 }, px: { xs: 2, md: 3 } }}>
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', lg: '1fr 1fr 1fr' }, gap: 3 }}>
        {TEAM_MEMBERS.map((member, index) => {
          ...
          return (
            <Box key={index} sx={{ borderRadius: 3, overflow: 'hidden', backgroundColor: 'background.paper', border: '1px solid', borderColor: 'divider', transition: 'all 0.25s ease', '&:hover': { transform: 'translateY(-4px)', borderColor: `${accent}40`, boxShadow: `0 12px 40px rgba(0,0,0,0.35), 0 0 0 1px ${accent}20` } }}>
              <Box sx={{ height: 110, background: `linear-gradient(135deg, ${accent}14, ${accent}06)`, borderBottom: `1px solid ${accent}18`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
