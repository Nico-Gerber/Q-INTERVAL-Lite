...
const OurTeam = () => (
  <Box sx={{ backgroundColor: 'background.default', minHeight: '100%', pb: 10 }}>
    ...
    >
      ...
      />
      <Typography variant="h3" sx={{ fontWeight: 700, mb: 1.5, color: 'text.primary' }}>Meet The Team</Typography>
    ...
    <Container maxWidth="lg" sx={{ mt: { xs: 4, md: 6 }, px: { xs: 2, md: 3 } }}>
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', lg: '1fr 1fr 1fr' }, gap: 3 }}>
        {TEAM_MEMBERS.map((member, index) => {
          ...
          return (
            <Box key={index} sx={{ borderRadius: 3, overflow: 'hidden', backgroundColor: 'background.paper', border: '1px solid', borderColor: 'divider', transition: 'all 0.25s ease', '&:hover': { transform: 'translateY(-4px)', borderColor: `${accent}40`, boxShadow: `0 12px 32px rgba(0,0,0,0.3), 0 0 0 1px ${accent}20` } }}>
              <Box sx={{ height: 110, background: `linear-gradient(135deg, ${accent}18, ${accent}06)`, borderBottom: `1px solid ${accent}18`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Avatar sx={{ width: 68, height: 68, backgroundColor: `${accent}14`, border: `2px solid ${accent}35`, color: accent, fontSize: '1.3rem', fontWeight: 800 }}>
                  {initials(member.name)}
                </Avatar>
              </Box>
              <Box sx={{ p: 3 }}>
                <Typography variant="h6" sx={{ fontWeight: 700, color: 'text.primary', mb: 0.25, fontSize: '1rem' }}>{member.name}</Typography>
                <Typography variant="caption" sx={{ color: accent, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', fontSize: '0.68rem' }}>{member.major}</Typography>
                <Divider sx={{ my: 2 }} />
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <School sx={{ fontSize: 14, color: 'text.disabled', flexShrink: 0 }} />
                    <Typography variant="caption" sx={{ color: 'text.disabled', fontSize: '0.75rem' }}>{member.studentId}</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Email sx={{ fontSize: 14, color: 'text.disabled', flexShrink: 0 }} />
                    <Typography variant="caption" sx={{ color: 'text.disabled', fontSize: '0.72rem', wordBreak: 'break-all' }}>{member.email}</Typography>
                  </Box>
                </Box>
                <Box sx={{ pt: 1.5, borderTop: '1px solid', borderColor: 'divider' }}>
                  <Typography variant="caption" sx={{ color: 'text.disabled', textTransform: 'uppercase', letterSpacing: '0.08em', fontSize: '0.65rem', fontWeight: 700, display: 'block', mb: 0.75 }}>Roles &amp; Responsibilities</Typography>
                  <Typography variant="body2" sx={{ color: 'text.secondary', lineHeight: 1.65, fontSize: '0.82rem' }}>{member.description}</Typography>
                </Box>
              </Box>
            </Box>
          );
