...
PCA_PATH             = MODELS_DIR / "qml_image_risk_pca.pkl"
SCALER_PATH          = MODELS_DIR / "qml_image_risk_scaler.pkl"
CANCER_WEIGHTS_PATH  = MODELS_DIR / "qml_cancer_classifier_weights.npz"
DENSITY_WEIGHTS_PATH = MODELS_DIR / "qml_density_classifier_weights.npz"
BIRADS_WEIGHTS_PATH  = MODELS_DIR / "qml_birads_classifier_weights.npz"
RISK_WEIGHTS_PATH    = MODELS_DIR / "qml_risk_regressor_weights.npz"
...
def _predict_single(image_bytes: bytes, filename: str = None) -> dict:
    # ── Cancer classification — use the SAME model as QMLPredictV2 ──
    clf_result = qml_classify_image(image_bytes, with_occlusion=False)
    cancer_pred = clf_result["result"]          # "Malignant" / "Benign" / "Normal"
    cancer_probs = clf_result["class_probabilities"]
    # Map to a risk score using your existing CANCER_RISK_MAP
    cancer_risk = sum(
        (float(p) / 100.0 if p > 1 else float(p)) * float(CANCER_RISK_MAP.get(cls, 0))
        for cls, p in cancer_probs.items()
    )

    # ── Density + BI-RADS still use their own circuits ──
    x = _preprocess(image_bytes)   # still needed for density/birads
    ...
    # ── Malignant gate — now consistent with the classifier ──
    if cancer_pred == "Malignant":
        result = {
            ...
            "feedback": (
                ...
                "is not applicable. Please seek medical review."
            ),
        }
        if filename:
    ...
    result = {
        ...
        "feedback":                msg,
        # keep highest_density/birads fields so handleExplain can read them
        "highest_density_risk_score": density_risk,
        "highest_birads_risk_score":  birads_risk,
    }
    if filename:
        result["filename"] = filename
    return result
