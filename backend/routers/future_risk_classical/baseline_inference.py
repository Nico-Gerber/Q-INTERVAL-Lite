...
DEFAULT_CHECKPOINT = os.path.join(PROJECT_DIR, "models", "lstm_baseline_15691343.pt")
DEFAULT_RESNET_WEIGHTS = os.path.join(
    PROJECT_DIR,
    "models",
    "torch_cache",
    "hub",
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Run the frozen classical future-risk baseline with influence estimates."
    )
    parser.add_argument("--input-json", required=True)
    parser.add_argument("--output-json", required=True)
    parser.add_argument("--checkpoint", default=DEFAULT_CHECKPOINT)
    parser.add_argument("--resnet-weights", default=DEFAULT_RESNET_WEIGHTS)
    parser.add_argument(
        "--device",
    ...
    for session_index, session in enumerate(sessions):
        ...
        for view_slot in available_views:
            ...
            row = {
                ...
                "exam_date": session["exam_date_text"],
                "view": view_slot,
                "image_path": session["images"][view_slot],
                "score_without_image": {},
                "signed_score_change": {},
...
def main():
    ...
    patient_id, sessions = load_request(args.input_json)
    device = select_device(args.device)
    resnet, lstm, checkpoint = load_models(
        args.checkpoint,
        args.resnet_weights,
        device,
    )
    feature_cache = extract_image_features(sessions, resnet, device)
    sequence, sequence_mask, days_before_anchor, recency_weights = build_sequence(
        sessions,
        feature_cache,
    )
    scores = predict_scores(sequence, sequence_mask, lstm, device)
        ...
        session_summary.append(
            {
                "session": index + 1,
                "exam_date": session["exam_date_text"],
                "days_before_anchor": int(days_before_anchor[index]),
                "recency_weight": float(recency_weights[index]),
                "available_views": [
                    slot for slot in VIEW_SLOTS if slot in session["images"]
                ],
                "num_available_views": len(session["images"]),
            }
        )

    score_map = {key: float(scores[index]) for index, key in enumerate(RISK_KEYS)}
    score_percent_map = {
        key: float(scores[index] * 100.0) for index, key in enumerate(RISK_KEYS)
    }
    monotonic = bool(np.all(np.diff(scores) >= 0.0))

    result = {
        "status": "success",
        "patient_id": patient_id,
        "model": {
            "name": "classical_lstm_baseline",
        ...
        "input_summary": {
            ...
            "num_images": len(feature_cache),
            "sessions": session_summary,
        },
        "model_scores": score_map,
        "model_scores_percent": score_percent_map,
        "scores_are_monotonic": monotonic,
        "image_influences": image_rows,
        "session_influences": session_rows,
        "skipped_image_influences": skipped,
        "explanation": {
            "method": "leave-one-image-out ablation",
            "session_method": "sum of image-level influence values within each session",
            "signed_change_definition": "full score minus score after removing the image",
            "positive_change": "The image increased the model score.",
            "negative_change": "The image reduced the model score.",
        },
        "warnings": [
            "Experimental research model. Not for clinical use.",
            "Scores are uncalibrated model outputs and are not clinical risk probabilities.",
            "Influence estimates describe model behaviour and do not establish causation.",
            "The baseline may produce non-monotonic scores across yearly horizons.",
            "Input images must follow the same cleaned-mammogram preprocessing domain used for training.",
        ],
    }
    ...
    temporary_path = args.output_json + ".tmp"
    ...
    print("Images:", len(feature_cache))
    print("Device:", device)
    print("Scores monotonic:", monotonic)
